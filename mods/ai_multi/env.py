...
class RhythmEnv(gym.Env):
    ...
    def __init__(self, parser=None, hw_mult=1.0, jitter=0.0):
        ...
        # dtype=np.int64 added in v1.2.2 (explicit over the default)
        self.action_space = spaces.Discrete(2, dtype=np.int64)
    ...
    def reset(self, seed=None, options=None):
        # super().reset() seeds self.np_random — must be called before any RNG use
    ...
    def step(self, action):
        ...
        # v1.0.0 naming: terminated = natural episode end, truncated = time limit
    ...
    def _get_obs(self):
        ...
        for note in self.notes:
            if 'hit' not in note and 'miss' not in note:
                ...
                # Use self.np_random (seeded by reset()) instead of np.random
                # so that seed= in reset() produces reproducible rollouts
                if self.jitter > 0:
                    noise = self.np_random.normal(0, self.jitter)
                    time_to_note += noise
